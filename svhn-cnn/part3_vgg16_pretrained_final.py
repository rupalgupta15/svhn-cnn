...
def vgg_build_model(load_weights_file=None):  # Xtrain, Ytrain, Xtest, Ytest, options
    model = VGG16(weights='imagenet', include_top=False)  # pretrained model
    model.summary()
    ...
    # x = Dense(1024, activation=tf.nn.relu, name='fc1')(x)
    # # x = Dense(options.fc, activation=tf.nn.relu, name='fc2')(x)
    # x = Dense(1024, activation=tf.nn.relu, name='fc2')(x)
    # x = Dropout(0.5)(x)
    # x = Dense(10, activation=tf.nn.softmax, name='predictions')(x)

    model = Flatten(name='flatten')(model)
    model = Dense(3072, activation=tf.nn.relu, name='fc1')(model)  # 2048
...
def vgg_train_model(Xtrain, Ytrain): # options,
    print('************* training VGG 16 pretrained model ***************')
    ...
    checkpoint = ModelCheckpoint(filepath='models/vgg_pretrain/pre_vgg-{epoch:03d}.h5', monitor='val_loss',
                                 save_best_only=True,
                                 save_weights_only=True, verbose=2)
    decrease_lr = ReduceLROnPlateau(monitor='loss', factor=0.1, patience=3, verbose=1, cooldown=0, min_lr=0.000001)
    early_stopping = EarlyStopping(monitor='loss', min_delta=0.0000001, patience=5, verbose=1, mode='auto')
    ...
    # Plotting the model: Uncomment above line and the code below if want to plot
    # The fit() method on a Keras Model returns a History object.
    # pre_vgg_hist_train = model.fit(Xtrain, [Y_num_dig, Y_dig1, Y_dig2, Y_dig3, Y_dig4, Y_dig5], batch_size=64,
    #           validation_split=0.2, epochs=epochs, verbose=2, callbacks=callbacks)
    ...
    # generate_acc_plots(pre_vgg_hist_train, 'dense_acc', 'val_dense_acc', "num_dig_acc","Number of Digits Accuracy")
    # generate_acc_plots(pre_vgg_hist_train, 'dense_1_acc', 'val_dense_1_acc', 'digit_1_acc', "Digit 1 Accuracy")
    # generate_acc_plots(pre_vgg_hist_train, 'dense_2_acc', 'val_dense_2_acc', 'digit_2_acc', "Digit 2 Accuracy")
    # generate_acc_plots(pre_vgg_hist_train, 'dense_3_acc', 'val_dense_3_acc', 'digit_3_acc', "Digit 3 Accuracy")
    # generate_acc_plots(pre_vgg_hist_train, 'dense_4_acc', 'val_dense_4_acc', 'digit_4_acc', "Digit 4 Accuracy")
    # generate_acc_plots(pre_vgg_hist_train, 'dense_5_acc', 'val_dense_5_acc', 'digit_5_acc', "Digit 5 Accuracy")
    #
    # # plots to create for single digit loss:
    # generate_loss_plots(pre_vgg_hist_train, 'dense_loss', 'val_dense_loss', 'num_dig_loss', "Number of Digits Loss")
    # generate_loss_plots(pre_vgg_hist_train, 'dense_1_loss', 'val_dense_1_loss', 'digit_1_loss', "Digit 1 Loss")
    # generate_loss_plots(pre_vgg_hist_train, 'dense_2_loss', 'val_dense_2_loss', 'digit_2_loss', "Digit 2 Loss")
    # generate_loss_plots(pre_vgg_hist_train, 'dense_3_loss', 'val_dense_3_loss', 'digit_3_loss', "Digit 3 Loss")
    # generate_loss_plots(pre_vgg_hist_train, 'dense_4_loss', 'val_dense_4_loss', 'digit_4_loss', "Digit 4 Loss")
    # generate_loss_plots(pre_vgg_hist_train, 'dense_5_loss', 'val_dense_5_loss', 'digit_5_loss', "Digit 5 Loss")
    #
    # # plots to create for model loss:
    # generate_model_loss_plot(pre_vgg_hist_train, 'loss', 'val_loss', 'model_loss', "Model Loss")
    #
    # # plots to create for single digit error:
    # generate_err_plots(pre_vgg_hist_train, 'dense_acc', 'val_dense_acc', "num_dig_error","Number of Digits Error")
    # generate_err_plots(pre_vgg_hist_train, 'dense_1_acc', 'val_dense_1_acc', 'digit_1_error', "Digit 1 Error")
    # generate_err_plots(pre_vgg_hist_train, 'dense_2_acc', 'val_dense_2_acc', 'digit_2_error', "Digit 2 Error")
    # generate_err_plots(pre_vgg_hist_train, 'dense_3_acc', 'val_dense_3_acc', 'digit_3_error', "Digit 3 Error")
    # generate_err_plots(pre_vgg_hist_train, 'dense_4_acc', 'val_dense_4_acc', 'digit_4_error', "Digit 4 Error")
    # generate_err_plots(pre_vgg_hist_train, 'dense_5_acc', 'val_dense_5_acc', 'digit_5_error', "Digit 5 Error")
...
#     fig.savefig('plots/vgg_pretrain/'+ filename+ '.png')
#     plt.close()
#
#
...
def load_mean():
    f = h5py.File("training_mean_std_info/pre_vgg_train_img_mean.h5", 'r')
    key = list(f.keys())[0]
...
def load_std():
    f = h5py.File("training_mean_std_info/pre_vgg_train_img_std.h5", 'r')
    key = list(f.keys())[0]
    return f[key]
...
def normalize_color_images(images):  # images are the xtrain kind of values
    ...
    # saved already
    save_mean_std(img_mean, 'training_mean_std_info/pre_vgg_train_img_mean.h5')
    save_mean_std(img_std, 'training_mean_std_info/pre_vgg_train_img_std.h5')
    return images
...
def test_model(Xtest, Ytest):
    model = vgg_build_model("./models/vgg_pretrain/pre_vgg-016.h5")
...
def main():
    ...
    # step 1: training
    # Xdata_norm, Ydata_norm = fetch_data()  # , test_Xdata_norm, test_Ydata_norm

...
import cv2
import h5py
from helpers import read_bounding_box_data, load_more_negative_images_64x64
...
def normalize_images(images):  # images are the xtrain kind of values and labels are y values
    ...
    save_mean_std(images_mean, 'training_mean_std_info/cnn1_train_img_mean.h5')
    save_mean_std(images_std, 'training_mean_std_info/cnn1_train_img_std.h5')
...
def neg_split(negatives):
    train_neg_idx = int(0.9 * len(negatives))
    return train_neg_idx


# method to train model
def data_prep():  # only for training
    print('------------------CNN1 Loading Data ------------------')
    ...
    Y_label_train = (new_y_labels[:, 0]).T

    digits_present = np.where(Y_label_train != 0)
    if Y_label_train[digits_present].shape[0] != 0:
        Y_label_train[digits_present] = 1

    digits_absent = np.where(Y_label_train == 0)
    if Y_label_train[digits_absent].shape[0] != 0:
        Y_label_train[digits_absent] = 0

    Xdata_norm = normalize_images(Xtrain)
    Ydata_norm = Y_label_train
...
def generate_test_data(negatives, train_idx):
    ...
    Y_label_test = (new_test_y_labels[:, 0]).T

    digits_present_test = np.where(Y_label_test != 0)
    if Y_label_test[digits_present_test].shape[0] != 0:
        Y_label_test[digits_present_test] = 1

    digits_absent_test = np.where(Y_label_test == 0)
    if Y_label_test[digits_absent_test].shape[0] != 0:
        Y_label_test[digits_absent_test] = 0

    test_Xdata_norm = test_normalize_images(Xtest)
    test_Ydata_norm = Y_label_test
